...
numarray = np.array([int(i) for i in digits])
begin = 0
max = 0
interval = 13
max_list = list()
...
while begin+interval < len(numarray):
    temp = numarray[begin: begin+interval]
    print(len(temp))
    ...
    if 0 not in temp:
        ...
        if sum > max:
            max = sum
            max_list = numarray[begin: begin+interval]
            max_idx = begin

    begin = begin + 1

print('Max = {}, idx = {}'.format(max, max_idx))
print(max_list)
print(array_prod([9, 9, 9, 9, 9, 9, 9, 9, 9 , 9, 9, 9, 9]))
